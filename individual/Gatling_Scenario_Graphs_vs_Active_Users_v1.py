...
def get_scenario_metrics(scenario_name, gatling_log_df):
    ...
    scenario_metrics_df["Users"] = scenario_metrics_df["Users"].ffill()
...
def validate_user_given_arguments(argv):
    ...
    version = '1.0'
    verbose = False
    output_graph_path = 'GatlingScenarioGraphs_Users.html'
    input_log = ""
...
def get_y_range_of_graph(scenario_metrics_df):
    ...
    # Right y-Axis Range
    right_y_axis_range = tmp_max_val_df['Users'].max() + 1

    # Left y-axis Range
    tmp_max_val_df = tmp_max_val_df.drop(["LocalTime"], axis=1)
    tmp_max_val_df = tmp_max_val_df.drop(["Users"], axis=1)
...
def plot_graph_by_transaction(scenario_metrics_df, overall_ninety_fifth_df, scenario):
    ...
    for col_name in scenario_metrics_df.columns:
        ...
        if col_name not in "LocalTime":
            if col_name in "Users":
                # Get the legend name
                legend_name = "All Users"

                # Setting the second y axis range name and range
                scenario_graph.extra_y_ranges = {"Active Users": Range1d(0, right_y_range)}

                # Adding the second axis to the plot.
                scenario_graph.add_layout(LinearAxis(y_range_name="Active Users", axis_label="Users"), 'right')
