...
# Description  : Calculates the RPM, NinetyFifth of the given scenario
# @param       : Scenario Name
# @param       : Gatling Log Dataframe
# @return      : Dataframe scenario_metrics_df with columns: [LocalTime, RPM, ${TransactionNames}]
...
def get_scenario_metrics(scenario_name, gatling_log_df):
    ...
    # Active Threads DF
    scenario_rpm_df = scenario_temp_df.loc[scenario_temp_df["Owner"] == "REQUEST"]

    # New Dataframe
    scenario_metrics_df = pd.DataFrame(columns=["LocalTime"])

    # Create temp DF for RPM
    scenario_rpm_temp_df = pd.DataFrame(columns=["LocalTime", "RPM"])
    
    # Start Begin and End Time
    begin_time = scenario_rpm_df["LocalTime"][scenario_rpm_df.index[0]]
    end_time = scenario_rpm_df["LocalTime"][scenario_rpm_df.index[-1]]

    # Entry when rpm would have been zero 1 sec prior to test
    scenario_rpm_temp_df.loc[0] = (begin_time - 1000, 0)

    # Just get Divisor to know how many loops to go through
    loop_count = (end_time - begin_time) // 60000

    for i in range(loop_count):
        end_time = begin_time + 60000
        tmp_df = scenario_rpm_df[
            (scenario_rpm_df["LocalTime"] >= begin_time) & (scenario_rpm_df["LocalTime"] <= end_time)]
        rpm = tmp_df["Scenario"].count()
        scenario_rpm_temp_df.loc[i + row_count] = [begin_time, rpm]
        begin_time = end_time

    # To calculate for the last TimeInterval
    end_time = scenario_rpm_df["LocalTime"][scenario_rpm_df.index[-1]]
    tmp_df = scenario_rpm_df[(scenario_rpm_df["LocalTime"] >= begin_time) &
                               (scenario_rpm_df["LocalTime"] <= end_time)]
    rpm = tmp_df["Scenario"].count()
    scenario_rpm_temp_df.loc[-1] = [begin_time, rpm]

    # Refresh the index
    scenario_rpm_temp_df = scenario_rpm_temp_df.reset_index(drop=True)
    scenario_rpm_temp_df = scenario_rpm_temp_df.applymap(str)

    # Merge the dataframe of RPM
    scenario_metrics_df = scenario_metrics_df.merge(scenario_rpm_temp_df, on='LocalTime', how='outer')
    ...
    scenario_metrics_df["RPM"] = scenario_metrics_df["RPM"].ffill()
...
def validate_user_given_arguments(argv):
    ...
    version = '1.0'
    verbose = False
    output_graph_path = 'GatlingScenarioGraphs_RPM.html'
    input_log = ""
...
def get_y_range_of_graph(scenario_metrics_df):
    ...
    # Right y-Axis Range
    right_y_axis_range = tmp_max_val_df['RPM'].max() + 1

    # Left y-axis Range
    tmp_max_val_df = tmp_max_val_df.drop(["LocalTime"], axis=1)
    tmp_max_val_df = tmp_max_val_df.drop(["RPM"], axis=1)
    ...
    for col_name in scenario_metrics_df.columns:
        ...
        if col_name not in "LocalTime":
            if col_name in "RPM":
                ...
                scenario_graph.extra_y_ranges = {"RPM": Range1d(0, right_y_range)}

                # Adding the second axis to the plot.
                scenario_graph.add_layout(LinearAxis(y_range_name="RPM", axis_label="Throughput (RPM)"), 'right')
