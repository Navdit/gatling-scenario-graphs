...
##################################################################################################################
# Function Name: get_right_yaxis_df
# Description  : Creates the dataframe for the given scenario based on the input given. As of now inputs permissible
#                are: Users, RPS, RPM and Errors
# @param       : Dataframe scenario_temp_df which is filtered based on scenario. Columns are: [Owner,Scenario,
#                Transaction_Name,Status, ResponseTime, LocalTime]
# @param       : Filter based on which we will plot right y-axis
# @param       : Interval at which the data is to be fetched or Granularity
# @return      : Dataframe scenario_metrics_df merged with filtered df: [LocalTime, ${filter}]
# Author       : Navdit Sharma
# Comments     : Created on 05/09/2018
##################################################################################################################
def get_right_y_axis_df(scenario_df, right_y_axis_filter, granularity):
    # Add Active Users
    right_y_axis_filter_list = "Users, Errors, RPS, RPM"
    active_users = 0
    scenario_right_y_axis_df = pd.DataFrame()

    # Errors
    if right_y_axis_filter in "Errors":
        print("In Errors")
        # Errors DF
        scenario_right_y_axis_df = scenario_df.loc[scenario_df["Status"] == "KO"]

        # Create temp DF for Errors
        scenario_right_y_axis_temp_df = pd.DataFrame(columns=["LocalTime", "Errors"])

    # Active Users
    elif right_y_axis_filter in "Users":
        print("In Users")
        # Active Threads DF
        scenario_right_y_axis_df = scenario_df.loc[scenario_df["Owner"] == "USER"]

        # Create temp DF for Users
        scenario_right_y_axis_temp_df = pd.DataFrame(columns=["LocalTime", "Users"])
    ...
    if not scenario_right_y_axis_df.empty:
        ...
        if right_y_axis_filter in "Users":
            scenario_right_y_axis_temp_df.loc[0] = (begin_time - 1000, 0)
        ...
        for i in range(loop_count):
            ...
            if right_y_axis_filter in "Errors":
                filter_value = tmp_df["Scenario"].count()
            elif right_y_axis_filter in "Users":
                filter_value = active_users + tmp_df["Scenario"].count()

            scenario_right_y_axis_temp_df.loc[i + 100000] = [begin_time, filter_value]
            begin_time = end_time
        ...
        tmp_df = scenario_right_y_axis_df[(scenario_right_y_axis_df["LocalTime"] >= begin_time) &
                                    (scenario_right_y_axis_df["LocalTime"] <= end_time)]
        filter_value = tmp_df["Scenario"].count()
        scenario_right_y_axis_temp_df.loc[-1] = [begin_time, filter_value]
...
# Description  : Calculates the Errors, NinetyFifth of the given scenario
# @param       : Scenario Name
...
def get_scenario_metrics(scenario_name, gatling_log_df):
    ...
    # Get the dataframe based on Filters of right y-axis
    scenario_metrics_temp_df = get_right_y_axis_df(scenario_temp_df, "Errors", 1000)

    # Merge the dataframe of Right Y-Axis
    scenario_metrics_df = scenario_metrics_df.merge(scenario_metrics_temp_df, on='LocalTime', how='outer')
...
def get_y_range_of_graph(scenario_metrics_df):
    ...
    # Right y-Axis Range
    right_y_axis_range = tmp_max_val_df['Errors'].max() + 1

    # Left y-axis Range
    tmp_max_val_df = tmp_max_val_df.drop(["LocalTime"], axis=1)
    tmp_max_val_df = tmp_max_val_df.drop(["Errors"], axis=1)
...
########################################################################################################################
def plot_graph_by_transaction(scenario_metrics_df, overall_ninety_fifth_df, scenario, right_y_axis_filter):
    ...
    scenario_graph = plot_new_graph('Time', 'datetime', 'Response Time (ms)', 1900, 400,
                                    left_y_range, 'below', tools_to_show)
    ...
    for col_name in scenario_metrics_df.columns:
        ...
        if col_name not in "LocalTime":
            # Get the column of Filter from dataframe
            if col_name in right_y_axis_filter:
                # Get the legend name
                if right_y_axis_filter in "Errors":
                    legend_name = "Errors"
                    right_y_range_name = "Errors"
                    right_y_axis_label = "Errors"
                    color_right_y_axis = "#d62728"

                # Setting the second y axis range name and range
                scenario_graph.extra_y_ranges = {"Errors": Range1d(0, right_y_range)}

                # Adding the second axis to the plot.
                scenario_graph.add_layout(LinearAxis(y_range_name=right_y_range_name, axis_label=right_y_axis_label),
                                          'right')
                ...
                plot_graph = scenario_graph.line('LocalTime',
                                                 ...
                                                 source=source,
                                                 line_width=2,
                                                 color=color_right_y_axis,
                                                 y_range_name=right_y_range_name,
                                                 name=col_name)
...
# @return      : Null
########################################################################################################################
def generate_graph(gat_log_df, graph_output_path, right_y_axis_filter, show_graph=False):
    ...
    for scnIndex in range(len(scenario_list)):
        ...
        complete_scenario_graph = plot_graph_by_transaction(scenario_metrics_df, overall_ninety_fifth_df,
                                                            scenario_name, right_y_axis_filter)
...
def main(argv):
    ...
    # Generate Graph based on right y-axis filter
    right_y_axis = "Errors"
    generate_graph(gat_log_graph_df, output_graph, right_y_axis, True)
