...
def get_scenario_metrics(scenario_name, gatling_log_df):
    ...
    # Errors DF
    scenario_errors_df = scenario_temp_df.loc[scenario_temp_df["Status"] == "KO"]
    # print(scenario_errors_df)

    # Create temp DF for Errors
    scenario_errors_temp_df = pd.DataFrame(columns=["LocalTime", "Errors"])

    if not scenario_errors_df.empty:
        # Start Begin and End Time
        begin_time = scenario_errors_df["LocalTime"][scenario_errors_df.index[0]]
        end_time = scenario_errors_df["LocalTime"][scenario_errors_df.index[-1]]
        ...
        for i in range(loop_count):
            ...
            tmp_df = scenario_errors_df[
                (scenario_errors_df["LocalTime"] >= begin_time) & (scenario_errors_df["LocalTime"] <= end_time)]
            errors = tmp_df["Scenario"].count()
            scenario_errors_temp_df.loc[i + row_count] = [begin_time, errors]
            begin_time = end_time

        # To calculate for the last TimeInterval
        end_time = scenario_errors_df["LocalTime"][scenario_errors_df.index[-1]]
        tmp_df = scenario_errors_df[(scenario_errors_df["LocalTime"] >= begin_time) &
                                   (scenario_errors_df["LocalTime"] <= end_time)]
        errors = tmp_df["Scenario"].count()
        scenario_errors_temp_df.loc[-1] = [begin_time, errors]

        # Refresh the index
        scenario_errors_temp_df = scenario_errors_temp_df.reset_index(drop=True)
        scenario_errors_temp_df = scenario_errors_temp_df.applymap(str)

    # Merge the dataframe of Errors
    scenario_metrics_df = scenario_metrics_df.merge(scenario_errors_temp_df, on='LocalTime', how='outer')
...
def validate_user_given_arguments(argv):
    ...
    version = '1.0'
    verbose = False
    output_graph_path = 'GatlingScenarioGraphs_Errors.html'
    input_log = ""
    ...
    for col_name in scenario_metrics_df.columns:
        ...
        if col_name not in "LocalTime":
            if col_name in "Errors":
                ...
                scenario_graph.add_layout(LinearAxis(y_range_name="Errors", axis_label="Errors"), 'right')
